...
//fin de buscador
  ...
  if(user === "Jefatura" || user === "Jefe de Proveeduría"){
    ...
    data.forEach(activo => {
      if (activo.estado == "Procesando movimiento") {
        ...
        <td>${activo.sucursalProvisional}</td>
        <td>${activo.tipo}</td>
        <td>${activo.coment}</td>
        ...
        `;
        sucursalNueva = activo.sucursalProvisional;
      }
      ...
      btnAceptar.forEach(aceptar => {
        aceptar.addEventListener('click', function () {
          ...
          const url = `/activo/modificarEstadoMovimiento/${codigoActivo}`;
          const data = {
        ...
        btnRechazar.forEach(aceptar => {
          aceptar.addEventListener('click', function () {
            const codigoActivo = this.getAttribute('data-id');
            const url = `/activo/modificarEstadoMovimientoRechazar/${codigoActivo}`;
            const data = {
              sucursalProvisional: "",
              estado: "Activa",
            };
