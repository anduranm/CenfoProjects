package tarea3.tarea3_v2;

import java.io.*;

public class AndresDuranMolina_Bono1_Tarea3 {
    static BufferedReader in = new BufferedReader(new InputStreamReader(System.in));
    static PrintStream out = System.out;

    public static void main(String[] args) throws IOException {

        int nivel = 0, team = -1, beca = -1, tipoBeca = 0; // entradas
        int i = 0; // contadores
        int encuesta = 1, j = 1, exit; // bucles
        ...
        while (encuesta == 1) {
            ...
            while (exit < 0 || exit > 1) {
                out.println(
                        "Opción Invalida.\nDesea continuar agregando algún otro estudiante? \n 0 para No | 1 para Si");
                exit = Integer.parseInt(in.readLine());
            }
            if (exit == 0) {
                encuesta--;
