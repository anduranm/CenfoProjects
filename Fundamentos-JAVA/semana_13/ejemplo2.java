...
public class ejemplo2 {
    ...
    public static void busquedaLineal(int[] numeros) throws IOException {
        int num, posicion = 0;
        ...
        for (int i = 0; i < numeros.length; i++) {
            if (numeros[i] == num) {
                posicion = i;
                bandera = true;
                break; // control de flujo aqui buscamos el primero
            }
        }
        if (bandera)

        {
            out.println("El numero " + num + " fue encontrado en la posicion " + (posicion + 1));
        } else {
            out.println("El numero " + num + " No fue encontrado en el arreglo.");
        }
    }
